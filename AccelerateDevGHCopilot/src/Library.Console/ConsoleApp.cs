...
async Task<ConsoleState> SearchBooks()
{
    ...
    // Find book by title (case-insensitive contains)
    var book = _jsonData.Books?.FirstOrDefault(b => !string.IsNullOrWhiteSpace(b.Title) && b.Title.Contains(bookTitle, StringComparison.OrdinalIgnoreCase));
    if (book == null)
    {
        Console.WriteLine($"No se encontró el libro \"{bookTitle}\".");
        return ConsoleState.PatronDetails;
    }

    // Find a BookItem for the book
    var bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == book.Id);
    if (bookItem == null)
    {
        Console.WriteLine($"{book.Title} no tiene ejemplares registrados.");
        return ConsoleState.PatronDetails;
    }

    // Check for an active loan (not returned)
